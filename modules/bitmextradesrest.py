...
class BitmexTradesREST(QAgentRequest):
		...
		if (response['result']):
			...
			if (int(response['http_code']) == 200):
				if (self.cut_off()):
					...
					if (parse_result['trades'] == 500):
						if (parse_result['last_timestamp'] > url['starttime']):
							url['start'] = 0
						else:
							url['start'] += 500
						url['starttime'] = parse_result['last_timestamp']
						url['url'] = self.config['url'].replace('[MARKET]', url['market'])
						url['url'] = url['url'].replace('[STARTTIME]', datetime.datetime.utcfromtimestamp(int(url['starttime'])/1000).isoformat())
						url['url'] = url['url'].replace('[ENDTIME]', datetime.datetime.utcfromtimestamp(int(url['endtime'])/1000).isoformat())
						url['url'] = url['url'].replace('[START]', str(url['start']))
		...
		for data in trades:
			if data['size'] > 0:
				...
				trade = {
					...
					"external_id": str(data['trdMatchID']),
					"price": data['price'],
					"quantity": quantity,
					"original_response": json.dumps(data)
				}
				# Convert UTC datetime to milliseconds since the Epoch
				utc_dt = datetime.datetime.strptime(data['timestamp'], '%Y-%m-%dT%H:%M:%S.%fZ')
				last_timestamp = round((utc_dt - datetime.datetime(1970, 1, 1)).total_seconds()*1000)
				trades_counter += 1
				#print(trade)
...
class BitmexTradesREST_start():
	def __init__(self, args):
		...
		#no limit for Bitmex, but let's keep 1 hr step
		...
		for market in markets:
			for ranges in time_ranges:
				...
				url = url.replace('[STARTTIME]', datetime.datetime.utcfromtimestamp(int(ranges[0])/1000).isoformat())
				url = url.replace('[ENDTIME]', datetime.datetime.utcfromtimestamp(int(ranges[1])/1000).isoformat())
